...
function doesItExist (str, type) {
  var bool
  if (type == 'id') {
    bool = !!$('#SFDSWFB-build').find('[data-id=' + str + ']')[0]
  } else if (type == 'name') {
    bool = !!$('#SFDSWFB-build').find('[data-name=' + str + ']')[0]
  }
  return bool
}
  ...
  for (i in saved.data) {
    ...
    for (key in saved.data[i]) {
      if (key == 'placeholder') {
        ...
        if (key == 'checkboxes') {
		  var checkboxes = typeof saved.data[i][key] === "string" ? saved.data[i][key].split("\n") : saved.data[i][key]
